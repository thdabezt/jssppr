...
from .cplex_cp import CplexCpModel, load_docplex_symbols
...
def prepare_cplex_cp2_plan(instance: Instance, domains: Domains) -> CplexCp2Plan:
...
def build_cplex_cp2_model(
    instance: Instance,
    domains: Domains,
    warm_start: Optional[Sequence[Sequence[int]]] = None,
    model_name: str = "JSSPPR_CPLEX_CP2",
) -> CplexCpModel:
    plan = prepare_cplex_cp2_plan(instance, domains)
    ...
    for entity in plan.real_entities:
        ...
        for phase in entity.suboperations:
            phase_interval = symbols["interval_var"](
                ...
                size=phase.duration,
                name="cp2_phase_e{}_p{}".format(phase.entity_id, phase.phase_index),
            )
            phase_intervals[phase.key] = phase_interval
            model.add(
                symbols["start_at_start"](master, phase_interval, int(phase.offset))
            )
            constraint_count += 1
    ...
    for entity in plan.dummy_entities:
        ...
        dummy = symbols["interval_var"](
            ...
            size=entity.duration,
            name="cp2_threshold_dummy_e{}".format(entity.entity_id),
        )
    ...
    makespan = symbols["max_of"](
        [
            symbols["end_of"](operation_intervals[int(entity.real_operation_id)])
            for entity in plan.real_entities
        ]
    )
